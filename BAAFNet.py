...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            makedirs(config.train_sum_dir) if not exists(config.train_sum_dir) else None
            time_stamp = time.strftime("%Y-%m-%d %X", time.localtime())
            log_fname = os.path.join(config.train_sum_dir,
                                     'log_train_' + dataset.name + str(dataset.val_split) + '_' + time_stamp + '.txt')
    ...
    def train(self, dataset):
        self.logger.info('****EPOCH {}****'.format(self.training_epoch))
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                if self.training_step % 50 == 0:
                    ...
                    self.logger.info(message.format(self.training_step, l_out, acc, 1000 * (t_end - t_start)))
                self.training_step += 1
                ...
                self.logger.info('Best m_IoU is: {:5.3f}'.format(max(self.mIou_list)))
    ...
    def evaluate(self, dataset):
        ...
        self.logger.info('eval accuracy: {}'.format(val_total_correct / float(val_total_seen)))
        self.logger.info('mean IOU:{}'.format(mean_iou))

        mean_iou = 100 * mean_iou
        self.logger.info('Mean IoU = {:.1f}%'.format(mean_iou))
